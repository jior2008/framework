...
import com.glaf.template.engine.TemplateEngine;
import com.glaf.template.engine.TemplateEngineFactory;
...
public class RendererContainer {
	...
	static {
		...
		String userFactories = CustomProperties.getString("userRendererFactories");
		...
		if (userFactories != null && userFactories.trim().length() > 0) {
			...
			for (int i = 0; i < uFactories.length; i++) {
				try {
					...
					logger.error("It appears that your factory does not implement " + "the RendererFactory interface",
							cce);
				} catch (Exception e) {
					logger.error("Unable to instantiate renderer factory [" + uFactories[i] + "]", e);
				}
			}
		}
		...
		if (rendererFactories.size() < 1) {
			logger.warn(
					"Failed to load any renderer factories.  " + "Rendering probably won't function as you expect.");
		}
	...
	public static Renderer getRenderer(Template template) {
		...
		throw new RuntimeException("No renderer found for template " + template.getTemplateId() + "!");
	}
	...
	public static void evaluate(String templateId, Map<String, Object> context, Writer writer) {
		Template template = TemplateContainer.getContainer().getTemplate(templateId);
		evaluate(template, context, writer);
	}

	public static void evaluate(String name, String content, String language, Map<String, Object> context,
			Writer writer) {
		if (language == null) {
			language = "freemarker";
		}
