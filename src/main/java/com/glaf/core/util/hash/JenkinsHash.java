...
public class JenkinsHash extends Hash {
	 ...
	 * @return a 32-bit value. Every bit of the key affects every bit of the return
	 *         value. Two keys differing by one or two bits will have totally
	 ...
	 *         <code>h = (h & hashmask(10));</code> In which case, the hash table
	 *         should have hashsize(10) elements.
	 * 
	 *         <p>
	 *         If you are hashing n strings byte[][] k, do it like this: for (int i
	 *         = 0, h = 0; i < n; ++i) h = hash( k[i], h);
	 * 
	 *         <p>
	...
	public int hash(byte[] key, int nbytes, int initval) {
		...
		for (; length > 12; offset += 12, length -= 12) {
			...
			/*
			 * mix -- mix 3 32-bit values reversibly. This is reversible, so any information
			 * in (a,b,c) before mix() is still in (a,b,c) after mix().
			 * 
			 ...
			 * 
			 * This was tested for: - pairs that differed by one bit, by two bits, in any
			 * combination of top bits of (a,b,c), or in any combination of bottom bits of
			 * (a,b,c). - "differ" is defined as +, -, ^, or ~^. For + and -, I transformed
			 * the output delta to a Gray code (a^(a>>1)) so a string of 1's (as is commonly
			 ...
			 * rot(c,16); c += b; \ b -= a; b ^= rot(a,19); a += c; \ c -= b; c ^= rot(b,
			 * 4); b += a; \ }
			 * 
			 * mix(a,b,c);
			 */
		 ...
		 * 
		 * #define final(a,b,c) \ { c ^= b; c -= rot(b,14); \ a ^= c; a -= rot(c,11); \
		 * b ^= a; b -= rot(a,25); \ c ^= b; c -= rot(b,16); \ a ^= c; a -= rot(c,4); \
